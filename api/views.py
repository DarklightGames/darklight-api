...
class UploadLogViewSet(viewsets.ModelViewSet):
    """
    Upload a log file
    """
    ...
    def upload(self, request, **kwargs):
        uploaded_log = self.get_object()

        if "file" not in request.data:
            raise ValidationError("No file sent.")

        file = request.data["file"]
        uploaded_log.file.save(file.name, file)
...
def parse_dt(timestr: str) -> datetime:
    return default_tzinfo(parser.parse(timestr), pytz.UTC)


# TODO: move
def normalize_log(data: bytes) -> bytes:
    return data.replace(b'\r', b'').replace(b'\n', b'')


# TODO: move
@transaction.atomic
def write_log(data: jm.Log, crc: int) -> None:
    def get_unique_damage_types(data: jm.Log) -> Set[str]:
        unique_damage_types: Set[str] = set()
    ...
    def get_unique_pawn_clases(data: jm.Log) -> Set[str]:
        unique_pawn_classes: Set[str] = set()
        for round_data in data.rounds:
            for frag_data in round_data.frags:
    ...
    def get_unique_construction_classes(data: jm.Log) -> Set[str]:
        unique_construction_classes: Set[str] = set()
        for round_data in data.rounds:
            for construction_data in round_data.constructions:
                unique_construction_classes.add(construction_data.class_name)
        return unique_construction_classes

    # aggregate lists from the data (do this before we do DB-heavy stuff)
    unique_damage_types = get_unique_damage_types(data)
    unique_pawn_classes = get_unique_pawn_clases(data)
    unique_construction_classes = get_unique_construction_classes(data)

    log: models.Log = models.Log()
    log.crc = crc
    log.version = data.version

    map_data: jm.Map = data.map
    ...
    for player_data in data.players:
        player_id: int = int(player_data.id)
        try:
            player: models.Player = models.Player.objects.get(id=player_id)
        except ObjectDoesNotExist:
            player: models.Player = models.Player(id=player_id)
            player.save()
    ...
    for round_data in data.rounds:
        round = models.Round(
            started_at = parse_dt(round_data.started_at),
            ended_at = None if round_data.ended_at is None else parse_dt(round_data.ended_at),
        ...
        models.Construction.objects.bulk_create(
            map(lambda construction_data: models.Construction(
                classname=construction_classes_by_class[construction_data.class_name],
...
class LogViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # test = request.data['log'].name
        # return Response({'result': test}, status=status.HTTP_201_CREATED, headers={})

        # get data
        data = normalize_log(request.data['log'].file.read())
        ...
        try:
            json_data: jm.Log = jm.Log.model_validate_json(data)
        ...
        json_data: jm.Log = jm.Log.model_validate_json(data)
        # except ValidationError:
        #     return Response(None, status=status.HTTP_400_BAD_REQUEST, headers={})

        # version gate
        if semver.compare(json_data.version[1:], '8.3.0') < 0:
            data = {'success': False, 'error': 'Log file version {} is unsupported.'.format(json_data.version[1:])}
            return JsonResponse(data, status=status.HTTP_406_NOT_ACCEPTABLE)

        # Ensure that this log hasn't been evaluated before.
        # TODO: Need a better way to do this. A log for the same round can be uploaded twice 
        #       if there are any slight changes to the file.
        try:
        ...
        # write the log
        task = AsyncTask(write_log, json_data, crc, task_name=request.data['log'].name)
        task.run()
...
#         json_log = json_models.Log.model_validate_json(data)
